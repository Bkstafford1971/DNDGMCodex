...
async function fetchData(target) {
    ...
    // Determine if target is a full URL (pagination) or an endpoint
    let url;
    ...
    try {
        ...
        // Toggle Filter UI: Only for Spells
        if (filterUI) {
...
// --- 2. SEARCH LOGIC (STRICT NAME FILTER) ---
async function handleSearch(event) {
    ...
    // Default search to monsters to maintain table format
    ...
    try {
        ...
        // STRICT FILTER: Name must actually contain the sequence
...
// --- 3. RENDERING ENGINE (TABLES FOR ALL CATEGORIES) ---
function renderResults(items, category, nextUrl, prevUrl) {
    ...
    // RACES TABLE
		...
		tableRows = items.map(item => {
			// 1. Remove Markdown characters (* and _)
			// 2. Remove the specific words "Size." and "Speed." if they appear at the start
...
async function viewDetails(route, slug) {
    ...
    try {
        ...
        // Special Formatting for Monsters
        if (route === 'monsters') {
            contentHtml += `
        ...
        // Special Formatting for Races (Combining all trait fields)
        else if (route === 'races') {
            const racialContent = [
                data.desc,
        ...
        // Default Formatting (Spells, Items, Feats)
        else {
...
// Call this on page load to ensure the start page is the first thing seen
window.onload = () => {
    showStartPage();
};